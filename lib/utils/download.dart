...
Future<bool> requestPermission() async {
  ...
  Future<bool> downloadData() async{
   ...
   List<dynamic> headerPasti = ['pranzo1','pranzo2','pranzo3','pranzo4','cena1','cena2','cena3','cena4','snack','colazione','from'];
   List<List<dynamic>>  rowPasti = [];
   rowPasti.add(headerPasti);
    List<Event> events = await db.eventDao.findAllEvents();
    events.forEach((element) {rowPasti.add([element.pranzo1,element.pranzo2,element.pranzo3,element.pranzo4,element.cena1,element.cena2,element.cena3,element.cena4,element.snack,element.colazione,element.from]);});
   if(rowPasti.length > 1){
    String eventsCsv = ListToCsvConverter().convert(rowPasti);
    final filePasti = File('$path/pasti.csv');
    filePasti.writeAsString(eventsCsv); didSuceed = true;}
